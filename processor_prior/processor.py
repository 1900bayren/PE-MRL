        ...
        for n_iter, batch in enumerate(train_loader):
            ...
            if (n_iter + 1) % log_period == 0:
                if cfg.SOLVER.SCHEDULER == 'cosine' or cfg.SOLVER.SCHEDULER == 'cosine-refine':
                    logger.info(
                        "Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, Loss_id: {:.3f}, Loss_tri: {:.3f}, Loss_msel: {:.3f}, Loss_ipil: {:.3f}, Loss_phy:{:.3e}, Acc: {:.3f}, Base Lr: {:.2e}"
                        .format(epoch, (n_iter + 1), len(train_loader),
                                loss_meter.avg, loss_meter_id.avg, loss_meter_tri.avg, loss_meter_msel.avg,
                                loss_meter_ipil.avg, loss_meter_phys.avg, acc_meter.avg, scheduler._get_lr(epoch)[0]))
                else:
                    logger.info(
                        "Epoch[{}] Iteration[{}/{}] Loss: {:.3f}, Loss_id: {:.3f}, Loss_tri: {:.3f}, Loss_msel: {:.3f}, Loss_ipil: {:.3f}, Loss_phy:{:.3fe}, Acc: {:.3f}, Base Lr: {:.2e}"
                        .format(epoch, (n_iter + 1), len(train_loader),
                                loss_meter.avg, loss_meter_id.avg, loss_meter_tri.avg, loss_meter_msel.avg,
                                loss_meter_ipil.avg, loss_meter_phys.avg, acc_meter.avg, scheduler.get_lr()[0]))
