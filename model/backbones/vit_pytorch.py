...
class Attention(nn.Module):
    ...
    def forward(self, x):
        # print(x.shape)
        B, N, C = x.shape
        # print(f"Input shape: {x.shape}")  # 打印输入的形状，帮助调试
        # if len(x.shape) == 4:  # 形状是 (B, C, H, W)，需要进行 flatten
        #     B, C, H, W = x.shape
        #     x = x.view(B, C, H * W).permute(0, 2, 1)  # 转换为 (B, N, C) 的形状
        # elif len(x.shape) == 3:  # 形状已经是 (B, N, C)
        #     pass
...
def vit_large_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0,
...
def vit_large_patch32_224_TransReID(img_size=(256, 128), stride_size=32, drop_rate=0.0,
                                   attn_drop_rate=0.0, drop_path_rate=0.1,
                                   camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = TransReID(
        img_size=img_size, patch_size=32, stride_size=stride_size,
        embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,
        camera=camera, view=view, drop_path_rate=drop_path_rate,
        drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
        norm_layer=partial(nn.LayerNorm, eps=1e-6),
...
def deit_large_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0,
                                    attn_drop_rate=0.0, drop_path_rate=0.1,
                                    camera=0, view=0, local_feature=False, sie_xishu=1.5, **kwargs):
    model = TransReID(
        img_size=img_size, patch_size=16, stride_size=stride_size,
